...
3. Install the runner's build toolchain: current-LTS Node.js (the server's system Node was an EOL 18.19.1, upgraded to 24.19.0 via NodeSource), a JDK (`openjdk-21-jdk-headless`, required by AGP 8.13 for the Android build — discovered missing during implementation, not anticipated at design time), the Android SDK + accepted licenses, `wine` (for the Windows cross-build target), `libarchive-tools` (provides `bsdtar` — electron-builder's `pacman` target shells out to `fpm`, which needs `bsdtar` to build the package's `.MTREE`; present by default on Arch, where this design was first verified, but not on Ubuntu — the first real end-to-end run against the runner failed with exit 127 until this was installed), and `pacman-package-manager` (apt, universe repo) for `repo-add`/`repo-remove` — confirmed available on this server (`archive.ubuntu.com/ubuntu noble/universe`) during implementation; it ships real `repo-add`, not just `pacman` itself, so no source build is needed.
...
- Should the `/cut-release` skill always build all three platforms, or accept a flag (e.g. `--only android`) to build a subset? This is a scope-of-first-version question for `tasks.md`/implementation, not something that changes the specs or the chosen approach — defaulting to "always build all three" for the first version is safe to decide during implementation.
