...
- Working storage on Android (the equivalent of the Electron working directory `packaged-project-files` introduced) is the app's private cache directory, populated by copying bytes out of picked `content://` URIs at ingest/open time, mirroring the existing "copy at ingest, not at save" design.
