...
- [`CmdOrCtrl+Z`/`CmdOrCtrl+Shift+Z` no longer perform native text-field undo/redo] → Because `Edit > Undo`/`Redo` are now custom `click` handlers instead of the `editMenu` role's `undo`/`redo` roles, Electron's accelerator dispatch routes the keystroke to the document-level history unconditionally — including while a text/number `<input>` (e.g. the template-name field, page-size fields) is focused. Pressing `Ctrl+Z` to fix a typo mid-edit now reverts the last *document* change instead of the keystroke, with no visual distinction between the two outcomes. Identified during adversarial PR review and accepted as a deliberate, disclosed trade-off rather than fixed with focus-aware muxing (checking `document.activeElement` and branching between `document.execCommand('undo')` and `temporal.getState().undo()`) — that mux was judged to add meaningful complexity/risk (deprecated `execCommand` API, uncertain reliability) for a scenario mitigated in practice by the app's inputs being short, easily retyped values rather than long-form text editing. Revisit if this becomes a real user complaint.
