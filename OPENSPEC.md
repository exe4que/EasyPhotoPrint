...
> **Decisión de diseño — El botón de modo `Simple` se deshabilita cuando el árbol activo ya no es representable en dos niveles.** La restricción de "solo dos niveles, hojas siempre `imageSlot`" de más arriba es una condición de *entrada*, no solo de edición dentro de `Simple`: si el `rootNode` de la página activa ya excede esa forma (un hijo directo que es a su vez un contenedor, o cualquier nodo a tres niveles de profundidad), el botón `simple` del selector de modos queda deshabilitado (grisado, no clickeable) hasta que el árbol vuelva a cumplir la condición — evita que "Simple" aplique su normalización destructiva (`normalizePageForSimpleMode`, que aplana el árbol) sobre una estructura que el usuario armó deliberadamente en `nested`. El chequeo es la función pura `isSimpleModeCompatible(rootNode)` en `packages/layout-engine` (reutilizable donde haga falta esta misma condición), no una re-implementación ad-hoc en el componente. Aplicar un template desde el `TemplateGallery` (`applyTemplate`) corre el mismo chequeo sobre el `rootNode` del template: si no es compatible con `Simple`, el modo cambia automáticamente a `nested` en el mismo gesto de "Apply to current page" — el usuario nunca queda en `Simple` viendo (o a punto de aplanar) un árbol que ya no le pertenece a ese modo.
...
> **Requisito funcional — Botón "Adjust to restore aspect ratio" cuando `lockedAxis === 'both'` distorsiona la imagen.** Cuando el usuario fija **ambos** ejes de `specificSizeMm` (`lockedAxis: 'both'`) con valores que ya no respetan el aspect ratio original de la imagen asignada (`widthMm/heightMm` del `ImageAsset`, con una tolerancia de 0.05mm sobre el alto esperado para absorber redondeo), el `PropertiesPanel` muestra, debajo de **cada** input (`Width` y `Height`), un botón de texto pequeño en amarillo con la leyenda "Adjust to restore aspect ratio". Presionarlo recalcula **ese** eje a partir del valor actual del otro eje y el aspect ratio original de la imagen (`widthMm = heightMm × aspectRatio` desde el botón de `Width`; `heightMm = widthMm / aspectRatio` desde el de `Height`), reutilizando la misma acción `setSlotSpecificSize` que un input manual — el resultado sigue teniendo `lockedAxis: 'both'` (ambos ejes siguen siendo explícitos, ahora consistentes entre sí), no vuelve a colapsar a un solo eje derivado. Si no hay imagen asignada al slot (no hay aspect ratio contra el cual comparar), o los ejes ya son consistentes, los botones no se muestran.
