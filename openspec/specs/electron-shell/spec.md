...
The application SHALL replace Electron's default application menu with a reduced menu template containing only `File` (with `New` and a platform-appropriate `Close`/`Quit` item), `Edit` (the default `editMenu` role), `Help`, and — on macOS — the app menu (`appMenu` role). The menu SHALL NOT include `View` or `Window` groups.
...
- **AND** that template SHALL contain a `File` menu with a `New` item (accelerator `CmdOrCtrl+N`) followed by a `Close` role on macOS or a `Quit` role elsewhere
