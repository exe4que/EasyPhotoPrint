...
- [x] 2.4 Apply a temporary amber flash to the wrapper div while `isHighlighted` is `true` — a custom `animate-flash-amber` utility (`tailwind.config.cjs`) that ramps a `box-shadow` glow to amber and back to nothing with a bounce-shaped keyframe, not a positional bounce of the panel itself.
...
- [x] 4.2 Verify via the real-Electron-under-Xvfb harness on desktop: banner is visible on a fresh/empty project, disappears after adding an image (stubbed the `dialog:open-images` IPC handler in Main for a deterministic fake asset, then clicked "Load images"), and clicking it flashes the Image Library panel's wrapper with the `animate-flash-amber` glow (appears immediately, gone after the ~1.2s duration) without moving the panel. Reappearing after emptying the library again could not be scripted — this app has no UI path to remove an image from the library once added — but the banner's visibility is a single reactive `imagePool.length === 0` selector with no separate code path or cached state for that direction, so it's covered by the same mechanism verified above.
